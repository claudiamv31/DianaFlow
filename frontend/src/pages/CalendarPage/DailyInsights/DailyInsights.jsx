  ...
  return (
    <div className="md:col-span-4 flex flex-col gap-6">
      <div className="bg-surface-container-highest/50 border border-outline-variant/30 p-6 rounded-lg animate-slide-in shadow-lg">
        ...
        <div className="mb-6 rounded-lg border border-outline-variant/30 p-4">
          <div className="flex items-center justify-between mb-3"><p className="text-xs font-bold uppercase tracking-widest text-primary">{t('symptoms.loggedTitle')}</p><button className="text-xs font-bold text-primary" onClick={onEditSymptoms}>{symptoms.length ? t('symptoms.edit') : t('symptoms.add')}</button></div>
          {symptoms.length ? <div className="flex flex-col gap-2">{symptoms.map((symptom) => <div key={symptom.id} className="flex items-center justify-between gap-2 text-sm"><span className="text-on-surface">{t(`symptom.${symptom.code}`)} <span className="text-xs text-on-surface-variant">{t(`severity.${String(symptom.severity).toLowerCase()}`)}</span></span><button aria-label={t('common.remove')} className="text-xs text-error" onClick={() => onDeleteSymptom(symptom)}><span className="material-symbols-outlined text-base">close</span></button></div>)}</div> : <p className="text-sm text-on-surface-variant">{t('symptoms.none')}</p>}
        </div>
